/* ============================================================
   SPENDLY — script.js  (Firebase Edition)
...
watchAuthState(
  ...
  async user => {
    ...
    // Load preferences + bulk data from Firestore
    ...
    // Real-time transaction listener (auto-updates UI on any change)
    ...
    // Update settings page email display
...
window.handleGoogleAuth = async function() {
  ...
  try {
    ...
    if (e.code !== 'auth/popup-closed-by-user') {
      showAuthMsg('login', 'error', e.code ? friendlyError(e.code) : e.message);
    }
  }
};
...
window.handleEmailLogin = async function() {
  ...
  if (!email)    { showAuthMsg('login', 'error', 'Please enter your email.');    return; }
  if (!password) { showAuthMsg('login', 'error', 'Please enter your password.'); return; }
...
window.handleEmailSignup = async function() {
  ...
  if (password.length < 6) { showAuthMsg('signup', 'error', 'Password must be at least 6 characters.'); return; }
  ...
  try {
    ...
    showAuthMsg('signup', 'error', e.code ? friendlyError(e.code) : e.message);
...
window.handleForgotPassword = async function() {
  const email = document.getElementById('login-email').value.trim();
  if (!email) { showAuthMsg('login', 'error', 'Enter your email above first, then click Forgot password.'); return; }
  try {
    await resetPassword(email);
    showAuthMsg('login', 'success', 'Password reset email sent! Check your inbox.');
  } catch (e) {
    showAuthMsg('login', 'error', e.code ? friendlyError(e.code) : e.message);
  }
};
...
function showAuthMsg(panelId, type, text) {
...
function unlockProtectedNav() {
  ...
  const sl = document.getElementById('storage-label');
  if (sl) sl.textContent = 'Data synced to cloud ☁️';
}
...
function lockProtectedNav() {
  ...
  const sl = document.getElementById('storage-label');
  if (sl) sl.textContent = 'Sign in to sync your data';
}
...
window.saveGoal = async function() {
  ...
  setSyncStatus('syncing');
  try {
    const id = await addGoal(_currentUser.uid, { name, target, saved, date, priority, desc });
...
window.saveRecurring = async function() {
  ...
  setSyncStatus('syncing');
  try {
    const id = await addRecurring(_currentUser.uid, { name, category: cat, frequency: freq, amount, date, type: currentRecType });
...
window.saveSettings = async function() {
  ...
  updateGreeting();
  try { await savePrefs(_currentUser.uid, DB.settings); } catch (_) { /* silent */ }
};
...
window.exportCSV = function() {
  ...
  download(`spendly_${todayStr()}.csv`, 'text/csv;charset=utf-8', rows.map(r => r.join(',')).join('\n'));
  ...
  chartInstances.pie = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: { backgroundColor:'#1a2035',titleColor:'#e2e8f0',bodyColor:'#94a3b8',borderColor:'rgba(255,255,255,.1)',borderWidth:1,
...
function renderBarChart() {
  ...
  chartInstances.bar = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: { legend:{labels:{color:'#94a3b8',font:{family:'Outfit',size:11}}}, tooltip:{backgroundColor:'#1a2035',titleColor:'#e2e8f0',bodyColor:'#94a3b8',borderColor:'rgba(255,255,255,.1)',borderWidth:1,callbacks:{label:c=>`${c.dataset.label}: ${fmt(c.raw)}`}} },
      scales: { x:{ticks:{color:'#64748b',font:{family:'Outfit'}},grid:{color:'rgba(255,255,255,.04)'}}, y:{ticks:{color:'#64748b',font:{family:'Outfit'},callback:v=>sym()+abbreviate(v)},grid:{color:'rgba(255,255,255,.04)'},beginAtZero:true} }
    }
  });
}
    ...
    return `
      <div class="budget-row">
        <div class="budget-meta">
          <div class="budget-name"><div class="legend-dot" style="background:${CAT_COLORS[cat]||'#64748b'}"></div>${cat}</div>
...
function renderAnalytics() {
  ...
  if (lctx) chartInstances.line = new Chart(lctx, {
    ...
    options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},
      plugins:{legend:{labels:{color:'#94a3b8',font:{family:'Outfit'}}},tooltip:{backgroundColor:'#1a2035',titleColor:'#e2e8f0',bodyColor:'#94a3b8',borderColor:'rgba(255,255,255,.1)',borderWidth:1,callbacks:{label:c=>`${c.dataset.label}: ${fmt(c.raw)}`}}},
      scales:{x:{ticks:{color:'#64748b',font:{family:'Outfit'}},grid:{color:'rgba(255,255,255,.04)'}},y:{ticks:{color:'#64748b',font:{family:'Outfit'},callback:v=>sym()+abbreviate(v)},grid:{color:'rgba(255,255,255,.04)'},beginAtZero:true}}}
  });
  const catT={};
...
/* ── Init ──────────────────────────────────────────────────── */
(function init() {
  // Show auth screen by default until Firebase confirms login state
  // watchAuthState above handles show/hide automatically
  const mDate = document.getElementById('m-date'); if (mDate) mDate.value = todayStr();
  const rDate = document.getElementById('r-date'); if (rDate) rDate.value = todayStr();
  setType('expense');
  console.log('Spendly v3.1 (Firebase) initialized ✓');
})();